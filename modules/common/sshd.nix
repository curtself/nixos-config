{ pkgs, ... }:
{
  services.openssh = {
    enable = true;
    openFirewall = true;
    settings = {
      PasswordAuthentication = false;
      PermitRootLogin = "no";
      AllowUsers = [ "curt" ];
      MaxAuthTries = 3;
    };
  };
  # Add the authorized keys for my user
  users.users."curt".openssh.authorizedKeys.keys = [
	"ssh-rsa "
	"ecdsa-sha2-nistp256 AAAAE2VjZHNhLXNoYTItbmlzdHAyNTYAAAAIbmlzdHAyNTYAAABBBFS2DKiv2EXBNTI3ZU2acati0GiAJdVh7BmTWLQVTOSX0jVZQNzc49eXhYDQhTTV8O/8NJRX3lLzcwrq3eZFmw0= id-phone-04f"
  ];

}
